...
export default class prefixSetCommand extends BaseCommand {
	...
	async run(context: Command.Context, args: param) {
		...
		if (guildData.Prefixes.length > 5)
			return context.editOrReply('⚠ | El servidor alcanzo el maximo de prefixes');
		if (guildData.Prefixes.includes(args.prefix))
			return context.editOrReply('⚠ | Ese prefix ya se encuentra establecido');
		if (args.prefix.length > 6)
			return context.editOrReply('⚠ | El prefix debe tener menos de 6 caracteres');
		await Model.findOneAndUpdate(
			{ ServerID: context.guildId },
			{ $push: { [`Prefixes`]: args.prefix } }
		);
		return context.editOrReply(
			`El prefix \`${args.prefix}\` fue añadido a la lista de prefixes del servidor`
		);
	}
}
