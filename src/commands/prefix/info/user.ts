...
export default class MemberCommand extends BaseCommand {
	...
	async run(context: Command.Context, args: param) {
		...
		const userWithBanner =
			User instanceof Structures.UserWithBanner
				? User
				: await context.rest.fetchUser(user.id);
    ...
    `,
		...
		if (isMember) {
			let JoinPosition = guild
				? `${getMemberJoinPosition(guild, member.id)}/${guild.memberCount}`
				: 'Sin Posición';
			...
			const roles = member.roles
				...
				.map((role: Structures.Role | string) => {
					if (role instanceof Structures.Role) {
						if (
							(role.isDefault || context.guildId !== member.guildId) &&
							role
						) {
							return ` `;
						}
