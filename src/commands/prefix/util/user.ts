...
import { getUserByText, fetchUserById, getMemberJoinPosition, permissionsToObject } from '../../../utils/functions'
...
export default class MemberCommand extends BaseCommand {
  ...
  async run(context: Command.Context, args:eval) {
    let {guild, message} = context; 

    let User = message.mentions.first() as Structures.Member || args.user ? await getUserByText(context, args.user) : undefined || context.member || context.user
    ...
    const userWithBanner = (User instanceof Structures.UserWithBanner) ? User : await context.rest.fetchUser(user.id);
    ...
    **AccentColor:** ${userWithBanner.accentColor ? intToHex(userWithBanner.accentColor) : 'Sin color'}
    `, true)

  if(isMember){
    let JoinPosition = guild ? `${getMemberJoinPosition(guild, member.id)[0]}/${guild.memberCount}` : 'Sin Posición'
    //Informacion de Miembro
    embed.addField('**En Servidor**', `
    ...
    **JoinPosition:** ${JoinPosition}
    ...
    const roles = member.roles
        ...
        let rolesText = `${roles.length > 8 ? `${roles.slice(0, 8).join(', ')} ${roles.length - 8}+` : roles.join(', ')}`
    ...
    let permissionsText = `${Perms.length > 8 ? `${Perms.slice(0, 8).join(', ')} ${Perms.length - 8}+` : Perms.join(', ')}`
    
  //accesorios facheros
    embed.addField('**Accesorios De Servidor**', `
