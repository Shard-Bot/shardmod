...
export default class logAddCommand extends BaseCommand {
	...
	async run(context: Command.Context, args: param) {
		...
		if (!target.isText)
			return context.editOrReply('⚠ | El canal debe ser de tipo Texto');
		...
		switch (event.toLocaleLowerCase()) {
			case 'entradas':
			case 'joins':
				if (document.Channels.JoinLog === target.id)
					return context.editOrReply(
						'El Canal ya se encuentra establecido en este evento'
					);
				await Model.findOneAndUpdate(
					{ ServerID: context.guildId },
					{ $set: { [`Channels.JoinLog`]: target.id } }
				);
				return context.editOrReply(
					`Canal \`${target.name}\` fue correctamente establecido en el evento JoinLog`
				);
				break;
			case 'salidas':
			case 'leaves':
			case 'exitlog':
				if (document.Channels.ExitLog === target.id)
					return context.editOrReply(
						'El Canal ya se encuentra establecido en este evento'
					);
				await Model.findOneAndUpdate(
					{ ServerID: context.guildId },
					{ $set: { [`Channels.ExitLog`]: target.id } }
				);
				return context.editOrReply(
					`Canal \`${target.name}\` fue correctamente establecido en el evento ExiLog`
				);
				break;
			case 'modactions':
			case 'modlog':
			case 'modlogs':
				if (document.Channels.ModLog === target.id)
					return context.editOrReply(
						'El Canal ya se encuentra establecido en este evento'
					);
				await Model.findOneAndUpdate(
					{ ServerID: context.guildId },
					{ $set: { [`Channels.ModLog`]: target.id } }
				);
				return context.editOrReply(
					`Canal \`${target.name}\` fue correctamente establecido en el evento ModLog`
				);
				break;
			case 'botactions':
			case 'botlogs':
			case 'botlog':
				if (document.Channels.BotLog === target.id)
					return context.editOrReply(
						'El Canal ya se encuentra establecido en este evento'
					);
				await Model.findOneAndUpdate(
					{ ServerID: context.guildId },
					{ $set: { [`Channels.BotLog`]: target.id } }
				);
				return context.editOrReply(
					`Canal \`${target.name}\` fue correctamente establecido en el evento BotLog`
				);
				break;
			case 'all':
				...
				for (const _module of BotLogs) {
					if (document.Channels[_module] !== target.id) {
						await Model.findOneAndUpdate(
							{ ServerID: context.guildId },
							{ $set: { [`Channels.${_module}`]: target.id } }
						);
				...
				if (success.length) {
					return context.editOrReply(
						`El Canal \`${
							target.name
						}\` correctamente establecido en el evento: \`${success.join(
							' '
						)}\``
					);
				} else {
					return context.editOrReply(
						'El Canal ya se encuentra establecido en cada evento'
					);
				}
				break;
			default:
