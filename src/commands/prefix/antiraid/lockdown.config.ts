...
export default class LockdownToggleCommand extends BaseCommand {
	constructor(client: CommandClient) {
		super(client, {
			...
			disableDm: true,
			args: [
				{ name: 'status', type: String, required: false, aliases: ['s', 'estado'] },
				{ name: 'target', type: String, required: false, aliases: ['t', 'objetivo'] },
				{ name: 'mode', type: String, required: false, aliases: ['m', 'modo'] },
				{ name: 'show', type: Boolean, required: false, aliases: ['display', 'panel'] },
			],
			metadata: {
				trustedOnly: true,
				description: 'Comando de configuracion del sistema Lockdown',
				usage: [`${COMMAND_NAME} [Modulo/Sistema] [-mode] [-target] [-status] [-show]`],
				example: [
	...
	async run(context: Command.Context, args: param) {
		...
		if (args.show) {
			...
			embed.setTitle(
				`Lockdown Status ${
					serverData.Modules.Lockdown.Enabled === true ? DiscordEmojis.ON : DiscordEmojis.OFF
				}`
			);
			embed.addField(
				'Status:',
				`• \`${serverData.Modules.Lockdown.Enabled === true ? 'Activado' : 'Desactivado'}\``,
				true
			);
			embed.addField('Modo:', `• \`${serverData.Modules.Lockdown.Mode}\``, true);
			embed.addField('Objetivo:', `• \`${serverData.Modules.Lockdown.Target}\``, true);
		...
		if (args.status) {
			...
			if (args.status.toLowerCase() === 'on' && serverData.Modules.Lockdown.Enabled === true)
				return context.editOrReply('⚠ | El sistema ya se encuentra encendido');

			if (args.status.toLowerCase() === 'off' && serverData.Modules.Lockdown.Enabled === false)
				return context.editOrReply('⚠ | El sistema ya se encuentra apagado');
			await Model.findOneAndUpdate(
				{ ServerID: context.guildId },
				{
					$set: {
						[`Modules.Lockdown.Enabled`]: args.status.toLowerCase() === 'on' ? true : false,
					},
				}
			);
			return context.editOrReply(
		...
		if (args.target) {
			...
			if (serverData.Modules.Lockdown.Target === args.target.toLowerCase())
				return context.editOrReply('⚠ | Ese objetivo ya se encuentra establecido');
			await Model.findOneAndUpdate(
				{ ServerID: context.guildId },
				{
					$set: {
