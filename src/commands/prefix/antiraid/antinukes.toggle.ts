...
export default class AntinukesToggleCommand extends BaseCommand {
	...
	async run(context: Command.Context, args: param) {
		...
		if (args.status) {
			...
			embed.setTitle(
				`Antinuke Status ${
					serverData.Modules.AntiNuker.Enabled === true
						? DiscordEmojis.ON
						: DiscordEmojis.OFF
				}`
			);
			embed.setFooter(
				`El sistema esta actualmente ${
					serverData.Modules.AntiNuker.Enabled === true
						? 'encendido'
						: 'apagado'
				}`
			...
			for (let module of Object.values(AntiNukesModules)) {
				if (serverData.Modules.AntiNuker.Config[module].Enabled === true)
					enabledEvents.push(`\`${module}\``);
				else disabledEvents.push(`\`${module}\``);
			}
			embed.addField(
				`• Eventos Activados [${enabledEvents.length}]:`,
				enabledEvents.join(' ') || '`No hay eventos activados`'
			);
			embed.addField(
				`• Eventos Desactivados [${disabledEvents.length}]:`,
				disabledEvents.join(' ') || '`No hay eventos desactivados`'
			);
		...
		await Model.findOneAndUpdate(
			...
			{
				$set: {
					[`Modules.AntiNuker.Enabled`]:
						serverData.Modules.AntiNuker.Enabled === true ? false : true,
				},
			}
		);
		return context.editOrReply(
			`El sistema Antinuke ha sido ${
				serverData.Modules.AntiNuker.Enabled === true ? 'desactivado' : 'activado'
			}`
		);
	}
}
